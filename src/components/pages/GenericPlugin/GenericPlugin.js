...
export default class GenericPlugin extends PureComponent {
  ...
  state = {
    ...
    columnNameSortBy: 'genericPluginsType',
    sortingOrder: 'asc',
    expandedPanel: 'all',
  ...
  filterAndSortGenericPlugin = (genericPlugins) => {
    ...
    const filterByGenericPluginTypePredicate = _.flow(_.get('genericPluginsType'), _.toLower, _.includes(nameShouldInclude));
  ...
  formValuesToString = (formValues, formName) => {
    ...
    const genericPluginsType = _.get(valuesNames.GENERIC_PLUGIN_TYPE)(formValues);
    ...
    if (formName === 'create') {
      ...
      return ({ genericPluginsType, note, author, source, userId });
    }
    if (formName === 'edit') {
      ...
      return ({ genericPluginsType, note, author, date, sourceId, source, userId });
    }
  };
