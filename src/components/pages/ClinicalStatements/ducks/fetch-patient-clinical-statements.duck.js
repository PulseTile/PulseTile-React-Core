...
import { fetchPatientClinicalStatementsDetailRequest } from './fetch-patient-clinical-statements-detail.duck';
...
export const FETCH_PATIENT_CLINICAL_STATEMENTS_UPDATE_REQUEST = 'FETCH_PATIENT_CLINICAL_STATEMENTS_UPDATE_REQUEST';
...
export const fetchPatientClinicalStatementsUpdateRequest = createAction(FETCH_PATIENT_CLINICAL_STATEMENTS_UPDATE_REQUEST);
...
export const fetchPatientClinicalStatementsUpdateEpic = (action$, store) =>
  action$.ofType(FETCH_PATIENT_CLINICAL_STATEMENTS_UPDATE_REQUEST)
    .mergeMap(({ payload }) =>
      ajax.getJSON(`${usersUrls.PATIENTS_URL}/${payload.userId}/clinicalStatements`, {
        headers: { Cookie: store.getState().credentials.cookie },
      })
        .flatMap((response) => {
          ...
          return [
            fetchPatientClinicalStatementsSuccess({ userId, clinicalStatements: response }),
            fetchPatientClinicalStatementsDetailRequest({ userId, sourceId }),
          ]
        })
        .catch(error => Observable.of(handleErrors(error)))
    );
