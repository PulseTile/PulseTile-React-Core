...
import ProblemsDiagnosisDetailHeader from '../header/ProblemsDiagnosisDetailHeader';

export default class ProblemsDiagnosisDetailPanel extends PureComponent {
  static propTypes = {
  ...
  render() {
    const { name, title, children, isOpen, onExpand, onEdit, editedPanel, onCancel, onSaveSettings, formValues, currentPanel, isCreatePanelVisible } = this.props;

    return (
      <div className={classNames('panel panel-secondary', { open: isOpen })}>
        <ProblemsDiagnosisDetailHeader onExpand={onExpand} name={name} title={title} currentPanel={currentPanel} />
